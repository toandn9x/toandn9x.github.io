...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // 1. Lấp đầy ống xăm ngay từ lúc khởi tạo
  ...
  if (container) {
    for (let i = 0; i < 35; i++) {
      ...
      // Random hoá vị trí và độ nghiêng
      const rot = (Math.random() - 0.5) * 40; 
      const leftPos = 5 + Math.random() * 85; // Phân bổ ngẫu nhiên từ 5% đến 90% chiều rộng ống
      const delay = Math.random() * 0.5;
      
      stick.style.left = `${leftPos}%`;
      stick.style.transform = `rotate(${rot}deg) translateY(${Math.random() * 10}px)`;
